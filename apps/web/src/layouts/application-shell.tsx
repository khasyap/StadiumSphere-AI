...
export function ApplicationShell() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 transition-colors duration-300 dark:bg-[#07111f] dark:text-slate-100">
      ...
      <motion.button whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.96 }} type="button" aria-label="Open quick action" onClick={() => show('Quick actions will grow with each future module.')} className="focus-ring fixed bottom-6 right-6 z-20 flex h-12 w-12 items-center justify-center rounded-2xl bg-cyan-500 text-slate-950 shadow-xl shadow-cyan-500/30"><Plus size={22} /></motion.button>
...
function Sidebar({ collapsed, mobileOpen, onCollapse }: { collapsed: boolean; mobileOpen: boolean; onCollapse(): void }) {
  return (
    <motion.aside animate={mobileOpen ? { x: 0, width: collapsed ? 80 : 288 } : { width: collapsed ? 80 : 288 }} transition={{ duration: .24, ease: 'easeOut' }} className={`fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-slate-200/80 bg-white/90 p-4 backdrop-blur-xl transition-transform duration-300 dark:border-white/10 dark:bg-[#0a1729]/90 lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'} ${collapsed ? 'lg:w-20' : 'lg:w-72'}`}>
...
function SearchDialog({ open, onClose }: { open: boolean; onClose(): void }) { const { show } = useToast(); if (!open) return null; return <div role="dialog" aria-modal="true" aria-label="Global search" className="fixed inset-0 z-50 grid place-items-start bg-slate-950/45 p-4 pt-24" onMouseDown={onClose}><div className="w-full max-w-xl rounded-2xl border border-slate-200 bg-white p-3 shadow-2xl dark:border-white/10 dark:bg-slate-900" onMouseDown={(event) => event.stopPropagation()}><input autoFocus aria-label="Search workspace" placeholder="Search the workspace…" className="focus-ring w-full rounded-xl bg-slate-100 px-4 py-3 text-sm outline-none dark:bg-white/10" /><button type="button" onClick={() => { show('Search is ready for future platform data.'); onClose(); }} className="focus-ring mt-3 w-full rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white dark:bg-white dark:text-slate-950">Search placeholder</button></div></div>; }
