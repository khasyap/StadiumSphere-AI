...
export interface FormField {
  ...
  placeholder?: string;
  required?: boolean;
  type?: 'datetime-local' | 'number' | 'select' | 'text';
}
...
export function ResourceDrawer({ children, onClose, open, title }: { children: ReactNode; onClose(): void; open: boolean; title: string }) {
  return <SlidePanel open={open} onClose={onClose} title={title}>{children}</SlidePanel>;
}
...
export function EmptyResource({ action, description, icon: Icon, title }: { action: ReactNode; description: string; icon: LucideIcon; title: string }) {
  return <EmptyState icon={<Icon size={22} />} title={title} description={description} action={action} />;
}
