...
export function PromptBox({ onSubmit }: { onSubmit(prompt: string): void }) { return <form onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); onSubmit(String(data.get('prompt') ?? '')); event.currentTarget.reset(); }} className="flex gap-2 rounded-2xl border border-slate-200 bg-white p-2 dark:border-white/10 dark:bg-white/5"><input name="prompt" aria-label="Ask StadiumSphere Assistant" placeholder="Ask about your operations…" className="focus-ring min-w-0 flex-1 bg-transparent px-3 text-sm outline-none" /><PremiumButton type="submit" variant="secondary" aria-label="Send prompt"><ArrowUpRight size={17} /></PremiumButton></form>; }
